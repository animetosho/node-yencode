...
static struct {
	...
	uint64_t ALIGN_TO(8, compact[256]);
#else
	#pragma pack(16)
...
#undef NEXT_DATA
			...
			p += lookups.BitsSetTable256inv[mask & 0xff] + lookups.BitsSetTable256inv[mask >> 8];
#else
			// lookup compress masks and shuffle
			vst1_u8(p, vtbl1_u8(
				vget_low_u8(dataA),
				vld1_u8((uint8_t*)(lookups.compact + (mask&0xff)))
			));
			p += lookups.BitsSetTable256inv[mask & 0xff];
			mask >>= 8;
			vst1_u8(p, vtbl1_u8(
				vget_high_u8(dataA),
				vld1_u8((uint8_t*)(lookups.compact + (mask&0xff)))
